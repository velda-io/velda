...
func NewLocalMiniRunner(zfsRoot string) *LocalMiniRunner {
	return &LocalMiniRunner{
		zfsRoot: zfsRoot,
	}
}
...
`, suiteName, configDir, veldaBin)
	...
	r.apiServer = exec.Command(fmt.Sprintf("%s/apiserver", bindir), "--config", configFile)
...
func (r *LocalMiniRunner) CreateTestInstance(t *testing.T, namePrefix string, image string) string {
