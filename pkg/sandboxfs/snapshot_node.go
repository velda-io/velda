...
// HandleDirDataNotification applies pushed directory entries to this node.
func (n *SnapshotNode) HandleDirDataNotification(notification *fileserver.DirDataNotification) {
	n.dirFetched.Do(func() {
		...
		n.client.DebugLog("Pre-loaded %d entries for inode %d", len(notification.Entries), notification.Ino)
	})
}
