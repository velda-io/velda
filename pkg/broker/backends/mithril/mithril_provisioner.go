...
type MithrilAutoPoolProvisioner struct {
	schedulerSet *broker.SchedulerSet
	cfg          *proto.MithrilAutoProvisioner
	brokerInfo   *agentpb.BrokerInfo
}

func (p *MithrilAutoPoolProvisioner) Run(ctx context.Context) {
	err := p.run(ctx)
	if err != nil {
		log.Panicf("Failed to run Mithril auto provisioner: %v", err)
	}
}

func (p *MithrilAutoPoolProvisioner) run(ctx context.Context) error {
	if p.brokerInfo == nil {
	...
	for _, detail := range p.cfg.PoolDetails {
		...
		// Create the spot bid template from common config and detail
		template := &proto.AutoscalerBackendMithrilSpotBid{
			...
			MaxSuspendedBids:     p.cfg.MaxSuspendedBids,
		...
		autoScaler.Backend = &proto.AutoscalerBackend{
			Backend: &proto.AutoscalerBackend_MithrilSpotBid{
				MithrilSpotBid: template,
			},
		}
		...
		log.Printf("Created Mithril auto-provisioned pool: %v (instance_type: %s, region: %s, limit_price: $%.2f/hr)",
			newPoolConfig.Name, detail.InstanceType, p.cfg.Region, detail.LimitPrice)
...
type MithrilAutoProvisionerFactory struct{}

func (*MithrilAutoProvisionerFactory) NewProvisioner(cfg *proto.Provisioner, schedulers *broker.SchedulerSet, brokerInfo *agentpb.BrokerInfo) (backends.Provisioner, error) {
	cfgMithril := cfg.GetMithrilAuto()
	return &MithrilAutoPoolProvisioner{
...
func (*MithrilAutoProvisionerFactory) CanHandle(cfg *proto.Provisioner) bool {
	return cfg.GetMithrilAuto() != nil
}

func init() {
	backends.RegisterProvisioner(&MithrilAutoProvisionerFactory{})
}
